```markdown
# Wordlist location of common user pass combinations `/home/kali/wordlists/seclists/Passwords/Default-Credentials/ftp-betterdefaultpasslist.txt`
```
```bash
...
**FileZilla:**  
Go to Settings > Connection > FTP and select “Active” under Transfer Mode.
...
**FileZilla:**  
Go to Settings > Connection > FTP and confirm “Passive mode” is enabled (EPSV is handled automatically).

**Command-line FTP:**
```bash
ftp> epsv # Enable EPSV explicitly
...
**FileZilla:**  
Go to Settings > Connection > FTP and disable EPSV explicitly (if available) or select “Use plain FTP (insecure)”.

**Command-line FTP:**
```bash
ftp> pasv # Enable PASV
ftp> dir
```
```
